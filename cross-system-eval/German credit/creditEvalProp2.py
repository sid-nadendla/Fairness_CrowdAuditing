...
def validate(df):
    ...
    for i in range(50):
        ...
        for _ in range(len(df)):
            ...
            if hammingDist(df["Credit Risk"][i], df["Credit Risk"][j]) > kappa + delta:
                gRhs.append(kappa + delta - (hammingDist(auditorOP(i, df), df["Credit Risk"][i]) + hammingDist(auditorOP(j, df), df["Credit Risk"][j])))
